...
//create a UUID to sign the JWT and then store it in the session
	...
	$token = (new Builder())
		->set($value, $content)
		//todo Change to appropriate Issuer/Audience when address changes
		->setIssuer("http://www.nmsavin.com")
		->setAudience("http://www.nmsavin.com")
		->setId(session_id())
		->setIssuedAt(time())
...
/**
 * verifies the X-JWT-TOKEN sent by react matches the JWT-TOKEN saved in this session.
 ...
 * JWT sent by react
 *
 * @return \Lcobucci\JWT\Token the JWT token supplied by react in the header
 */
 ...
 *
 * @param \Lcobucci\JWT\Token $headerJwt X-JWT-TOKEN sent by react
 */
